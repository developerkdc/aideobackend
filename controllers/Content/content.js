...
export const getContentByCreatorId = catchAsyncErrors(
    async (req, res, next) => {
        ...
        const content = await Content.find({ creatorId: currentUser._id })
            .select("-contentData")
            .populate({
                path: "creatorId",
                select: "name _id email",
...
export const updateContentVerifyStatus = catchAsyncErrors(
    async (req, res, next) => {
        ...
        try {
            ...
            return next(
                new Errorhandler("Failed to update content status", 500)
            );
        }
    }
);
    ...
    async (req, res, next) => {
        ...
        try {
            ...
            if (verifiedContentIds.length === 0) {
                return res
                    .status(400)
                    .json({ error: "No verified content found" });
            }
            ...
            if (status === "Scheduled" && date) {
                ...
                return res.status(200).json({
                    message:
                        "Content status will be updated on the scheduled date",
                });
            }
...
export const allocateUserToVerify = catchAsyncErrors(async (req, res, next) => {
    ...
    const updatedContent = await Content.findByIdAndUpdate(
        ...
        {
            $set: {
                "allocated.0.allocatedBy": new mongoose.Types.ObjectId(
                    allocatedBy
                ),
                "allocated.0.allocatedTo": new mongoose.Types.ObjectId(
                    allocatedTo
                ),
            },
        },
...
export const getContentByTags = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!content || content.length === 0) {
        return next(
            new Errorhandler("No content with these tag IDs found", 400)
        );
    }
